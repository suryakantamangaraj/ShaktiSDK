...
typedef void (*interrupt_ptr)(int);
interrupt_ptr g_ext_interrupt_handlers[PLIC_NUM_INTERRUPTS];
...
void mei_isr(){
  printf("PLIC_HANDLER:Came to local Interrupt Service Routine\n");
  plic_source int_num  = PLIC_claim_interrupt(&g_plic);
  if ((int_num >=1 ) && (int_num < PLIC_NUM_INTERRUPTS)) {
  printf("PLIC_HANDLER:Going to Interrupt Service Routine for the Bit %"PRIu32"\n",int_num);
...
int main(int argc, char **argv)
{
  ...
  {
    g_ext_interrupt_handlers[gisr] = interrupt_handler;
  }
...
for(int i=0;i<PLIC_NUM_INTERRUPTS;i++)
  PLIC_enable_interrupt (&g_plic, i);
...
//  PLIC_set_priority(&g_plic, PWM0_INT_BASE + 0,5);
  ...
  // would win
  //
  PLIC_set_priority(&g_plic,25,8);
  PLIC_set_priority(&g_plic,26,8);
  PLIC_set_priority(&g_plic,27,8);
  PLIC_set_priority(&g_plic,28,8);
  PLIC_set_priority(&g_plic,29,8);
  PLIC_set_priority(&g_plic,30,8);
